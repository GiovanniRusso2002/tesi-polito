...
\subsubsection{Container Orchestration: Kubernetes vs ECS Fargate}
...
From a migration perspective, ECS Fargate enabled a smoother transition from the original Docker Compose deployment model running on DigitalOcean Droplets because applications could continue to be packaged as standard containers without requiring a complete redesign of the runtime environment.
...
Another alternative consisted of preserving the existing infrastructure provider and progressively improving the deployment model while continuing to operate on DigitalOcean Droplets. This approach would have minimized migration effort in the short term and leveraged existing operational knowledge. However, it would not have provided the same level of managed-service integration, infrastructure automation, and cloud-native capabilities offered by AWS. As a consequence, it was considered less suitable for supporting the long-term modernization objectives of the platform.
...
The migration involved replacing a virtual-machine-based infrastructure hosted on DigitalOcean Droplets with a cloud-native architecture built on AWS managed services. Rather than performing a disruptive lift-and-shift migration, the strategy focused on progressively modernizing infrastructure, deployment workflows, and data-management components while maintaining operational continuity.

AWS-managed services were adopted for runtime execution, routing, storage, authentication, and observability, with ECS Fargate selected over Kubernetes as the container runtime platform (see Section~\ref{sec:runtime-options}). Terraform became the primary infrastructure control layer responsible for reproducible provisioning and environment governance.

The migration strategy also introduced a progressive transition from Airtable to PostgreSQL with coexistence-oriented validation mechanisms. Instead of replacing the legacy backend immediately, the migration process relied on incremental rollout and controlled verification workflows capable of reducing operational risk.
...
Another key motivation was the quality of the managed database offering, in particular Amazon Aurora PostgreSQL, which combines low-maintenance operational requirements with strong reliability and global scalability characteristics. AWS was also considered highly developer-friendly, with broad customizability enabled by Lambda-based logic and service integrations that fit the operating model of a SaaS company requiring implementation flexibility.
