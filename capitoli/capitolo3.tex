...
Before the migration to AWS, Outsight Cloud relied on a containerized architecture hosted on DigitalOcean Droplets and designed to support rapid feature delivery and operational flexibility during the early stages of product development.

The platform managed LiDAR-related workflows and assets through a web application and backend API stack composed of several interconnected services deployed on DigitalOcean virtual machines. The frontend was implemented as a Vue 3 Single Page Application (SPA), while backend logic was handled by a Node.js/Express API service. Session management relied on Redis, and the overall deployment was orchestrated through Docker Compose with Traefik acting as reverse proxy and HTTPS entry point.
...
Although this architecture enabled fast iteration and reduced initial infrastructure complexity, it combined self-managed infrastructure hosted on DigitalOcean with multiple external service dependencies. Over time, this approach introduced increasing dependency concentration, operational coupling, and infrastructure management challenges that complicated scalability, maintainability, and long-term platform evolution.
...
More broadly, the migration formed part of a larger modernization initiative aimed at replacing the existing DigitalOcean-based virtual-machine deployment model with an AWS-native architecture built around managed services, Infrastructure as Code, and containerized workloads.
