...
\begin{itemize}
    \item establish cloud infrastructure on AWS;
    \item formalize reproducible provisioning with Terraform;
    \item migrate data from Airtable to PostgreSQL;
    \item validate migrated data, application behavior, and deployment correctness before production switchover.
\end{itemize}
...
The infrastructure migration was executed as a first step, allowing AWS environments, networking components, load balancing, container execution services, and deployment workflows to be validated independently before the Airtable-to-PostgreSQL data transition took place.

Figure~\ref{fig:migration-incremental-flow} summarizes this execution model by showing how the migration moved from the legacy baseline toward the target AWS cloud-native architecture through separate infrastructure, deployment, data-migration, and validation tracks.
...
Morris identifies three mutually reinforcing core practices for IaC: defining everything as code, continuously testing and delivering all work in progress, and building small loosely coupled pieces~\cite{morris2021iac}. Code defined as declarative artifacts is reusable, consistent, and transparent: everyone can see how infrastructure is built, changes are auditable, and environments can be recreated deterministically.
...
Listing~\ref{lst:terraform-ecs} shows a simplified Terraform resource definition used to deploy a backend container as an ECS Fargate service.
...
\subsection{Rationale for Adopting Terraform}
...
\begin{itemize}
    ...
    \item verify migration correctness at each checkpoint before production switchover.
\end{itemize}
