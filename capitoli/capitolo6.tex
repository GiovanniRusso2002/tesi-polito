...
Listing~\ref{lst:terraform-ecs} shows a simplified Terraform resource definition used to deploy a backend container as an ECS Fargate service. Through declarative configuration files, containerized workloads previously orchestrated through Docker Compose on DigitalOcean Droplets could be provisioned reproducibly in AWS while preserving the same packaging model.
...
This separation was particularly important during the migration from the existing DigitalOcean-based infrastructure because it allowed AWS environments to be validated independently before production workloads were progressively redirected to the new platform.

\section{Infrastructure Migration Execution}
\label{sec:infra-migration-exec}
While Chapters~\ref{chap:baseline} and~\ref{chap:architecture} describe respectively the legacy platform and the target AWS design, this section documents how the transition from DigitalOcean to AWS was executed in practice. The objective was to replace the previous Docker Compose and Traefik deployment model with a reproducible cloud-native foundation without forcing a simultaneous application and data cutover.
...
The initial Terraform implementation provisioned the main building blocks of the target platform, including ECS Fargate services for the frontend and backend containers, an Application Load Balancer for public routing, Route~53 records for DNS management, ACM certificates for HTTPS, CloudWatch log groups for centralized logging, and S3 together with CloudFront for simulation file storage and delivery. This stack established the first operational AWS environment capable of replacing the legacy hosting model on DigitalOcean Droplets.

\subsection{Routing, Application Adaptation, and Container Deployment}
A key difference between the legacy and target deployment models concerned request routing. On DigitalOcean, Traefik acted as reverse proxy and HTTPS entry point for containerized services. In the AWS architecture, routing responsibilities were transferred to the ALB and expressed explicitly in infrastructure and application configuration.

The frontend was exposed at the root path, while backend APIs were served under \texttt{/api}. To support this model without relying on proxy-level path rewriting, the Express backend was adapted so that API routes could be mounted under \texttt{/api} while preserving the \texttt{/status} endpoint for health checks. This change simplified the deployment topology and made routing behavior explicit and verifiable in the target environment.

Container images continued to be distributed through GitLab Container Registry. To allow ECS tasks to pull private images securely, AWS Secrets Manager was integrated so registry credentials could be retrieved at runtime without storing passwords directly in Terraform configuration or state files. Frontend and backend workloads were then deployed as separate ECS Fargate services, preserving the container packaging model already used in the legacy platform.
...
\begin{itemize}
    \item AWS infrastructure provisioning and controlled transition from the DigitalOcean baseline;
